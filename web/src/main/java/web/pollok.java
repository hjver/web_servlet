...
public class pollok extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String etc[] = request.getParameterValues("etc"); //같은이름 name을 가진 check box
		//ArrayList<String> etc =new ArrayList<String>(Arrays.asList("etc"));
		PrintWriter pw = response.getWriter();		
		try {
			request.setAttribute("etc", etc);
			request.setAttribute("subject", subject); //view 출력메소드
